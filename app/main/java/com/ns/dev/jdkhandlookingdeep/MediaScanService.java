...
public class MediaScanService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        new Thread(() -> {
            scanMedia();
            // After scan, optionally save to file
            saveMediaListToFile();
    ...
    private void scanAudio() {
        ...
        String selection = MediaStore.Audio.Media.MIME_TYPE + " IN ('audio/mpeg') OR " +
                MediaStore.Audio.Media.MIME_TYPE + " LIKE 'audio/mp4'";
    ...
    private void saveMediaListToFile() {
        try (FileOutputStream fos = openFileOutput(SCAN_FILE, MODE_PRIVATE);
             OutputStreamWriter writer = new OutputStreamWriter(fos)) {
            gson.toJson(mediaList, writer);
        } catch (Exception e) {
            Log.e(TAG, "Failed to save media list", e);
        }
    }
